...
class DDNParameterSearch:
    def __init__(
        ...
        self.cores = cores
        self.cv_max_edge_per_node = cv_max_edge
    ...
    def run_cv_joint(self):
        ...
        val_err, _, _ = cv_two_lambda(
            ...
            cores=self.cores,
            n_max=self.cv_max_edge_per_node,
        )
...
def calculate_regression(data, topo_est, cores=8, n_max=(20,)):
    """Linear regression
    ...
    n_fea = data.shape[1]
    if len(n_max) == 1:
        n_max_lst = np.zeros(n_fea) + n_max[0]
    else:
        n_max_lst = n_max
...
def cv_two_lambda(
    ...
    return val_err, (lambda1_lst, lambda2_lst), dbg_info
